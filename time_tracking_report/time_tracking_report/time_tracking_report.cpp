...
int main()
{
    ...
    {
        int j = 0;
        while (getline(filein, line))//parsing info from file
        {  
            if (j != 0)
            {
                row.clear();
                ...
                {
                    if (i == 0 || i == 6 || i == 7)//parsing of only needed columns
                    {
                        row.push_back(word);
                    }
                    i++;
    ...
    for (int i = 0; i < data.size(); i++)//preparing data before sorting
    {
    ...
    data.insert(data.begin(), row);//adding columns' headers

    fstream fout("result.csv", ios::out);
    for (int i = 0; i < data.size(); i++)//writing report to file result.csv
    {
